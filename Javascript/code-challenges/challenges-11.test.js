...
function transformToLis(obj){
  ...
  const lis = pairs.map((pair) => `<li>${pair[0]}: ${pair[1]}</li>`);
  return lis;
}
...
const count = (target, input) => {
  return input.reduce((acc, arr) => {
    return (
        acc +
        arr.reduce((subAcc, curr) => {
            return subAcc + (curr === target ? 1 : 0);
        }, 0)
    );
}, 0);
};
...
let findShortest = (data) => {
  const shortest = data
  .filter((char) => char.height !== "unknown")
  .reduce((acc, char) => {
      if (parseInt(char.height) < parseInt(acc.height)) {
          return char;
      }
      return acc;
  }, { height: "1000" });
